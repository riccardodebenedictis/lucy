...
namespace cg {
	...
	std::vector<flaw*> reusable_resource::get_flaws() {
		...
		if (to_check.empty()) {
			...
			for (const auto& a : atoms) {
				...
				if (graph.core::sat.value(graph.set.allows(a.first->state, *atom::active)) == True) {
			...
			for (const auto& rr : instances) {
				...
				for (const auto& p : pulses) {
					...
					if (graph.la.value(resource_usage) > graph.la.value(capacity->l)) {
						// we have a peak..
	...
	bool reusable_resource::new_fact(atom& a) {
		...
		set_var(graph.set.allows(a.state, *atom::active));
		...
		if (!graph.core::sat.new_clause({ lit(graph.set.allows(a.state, *atom::unified), false) })) {
			return false;
		}
