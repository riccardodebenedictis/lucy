...
bool propositional_state::new_goal(atom &atm)
{
    atoms.push_back({&atm, new ps_atom_listener(*this, atm)});
    to_check.insert(&atm);
    return true;
...
bool propositional_state::ps_resolver::apply() { return graph.core::sat.new_clause({lit(chosen, false), to_do}); }
