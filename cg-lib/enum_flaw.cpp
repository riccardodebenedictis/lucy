...
enum_flaw::choose_value::choose_value(causal_graph &graph, enum_flaw &enm_flaw, set_item &val) : resolver(graph, graph.set_th.allows(enm_flaw.e_itm.ev, val), lin(1.0 / graph.set_th.value(enm_flaw.e_itm.ev).size()), enm_flaw), v(enm_flaw.e_itm.ev), val(val) {}
