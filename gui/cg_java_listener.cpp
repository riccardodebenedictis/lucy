...
void cg_java_listener::flaw_created(const cg::flaw &f)
{
    ...
    env->CallVoidMethod(cg_object, f_created, reinterpret_cast<jlong>(&f), causes_array, env->NewStringUTF(f.get_label().c_str()), f.get_cost(), graph.core::sat.value(f.get_phi()));
}
void cg_java_listener::flaw_state_changed(const cg::flaw &f) { env->CallVoidMethod(cg_object, f_state_changed, reinterpret_cast<jlong>(&f), graph.core::sat.value(f.get_phi())); }
...
void cg_java_listener::resolver_created(const cg::resolver &r) { env->CallVoidMethod(cg_object, r_created, reinterpret_cast<jlong>(&r), reinterpret_cast<jlong>(&r.get_effect()), env->NewStringUTF(r.get_label().c_str()), graph.core::sat.value(r.get_rho())); }
void cg_java_listener::resolver_state_changed(const cg::resolver &r) { env->CallVoidMethod(cg_object, r_state_changed, reinterpret_cast<jlong>(&r), graph.core::sat.value(r.get_rho())); }
