...
void cg_java_listener::flaw_created(const cg::flaw &f)
{
    ...
    gui.env->CallVoidMethod(gui.cg_object, gui.f_created, reinterpret_cast<jlong>(&f), causes_array, gui.env->NewStringUTF(f.get_label().c_str()), f.get_cost(), slv.core::sat.value(f.get_phi()));
}
void cg_java_listener::flaw_state_changed(const cg::flaw &f) { gui.env->CallVoidMethod(gui.cg_object, gui.f_state_changed, reinterpret_cast<jlong>(&f), slv.core::sat.value(f.get_phi())); }
...
void cg_java_listener::resolver_created(const cg::resolver &r) { gui.env->CallVoidMethod(gui.cg_object, gui.r_created, reinterpret_cast<jlong>(&r), reinterpret_cast<jlong>(&r.get_effect()), gui.env->NewStringUTF(r.get_label().c_str()), slv.core::sat.value(r.get_rho())); }
void cg_java_listener::resolver_state_changed(const cg::resolver &r) { gui.env->CallVoidMethod(gui.cg_object, gui.r_state_changed, reinterpret_cast<jlong>(&r), slv.core::sat.value(r.get_rho())); }
