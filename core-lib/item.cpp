...
namespace lucy {
	...
	var arith_item::eq(item & i) noexcept {
		if (this == &i) {
			...
			return cr.sat.new_conj({ lit(cr.la.new_leq(l, ae->l), true), lit(cr.la.new_geq(l, ae->l), true) });
		}
		else {
			return FALSE;
		}
	...
	bool arith_item::equates(const item & i) const noexcept {
		if (this == &i) {
			...
			interval c_val = cr.la.bounds(l);
			interval i_val = cr.la.bounds(ae->l);
	...
	expr enum_item::get(const std::string & name) const {
		if (items.find(name) == items.end()) {
			std::unordered_set<set_item*> vs = cr.set.value(ev);
			if (vs.size() == 1) {
				...
				for (unsigned int i = 0; i < c_vals.size(); i++) {
					bool af = cr.sat.eq(lit(cr.set.allows(ev, *c_vals[i]), true), lit(cr.set.allows(e->ev, *f_vals[i]), true));
					assert(af);
				}
	...
	var enum_item::eq(item & i) noexcept {
		if (this == &i) {
			...
			return cr.set.eq(ev, ee->ev);
		}
		else {
			return cr.set.allows(ev, i);
		}
	}
	...
	bool enum_item::equates(const item & i) const noexcept {
		if (this == &i) {
			...
			std::unordered_set<set_item*> c_vals = cr.set.value(ev);
			std::unordered_set<set_item*> i_vals = cr.set.value(ei->ev);
